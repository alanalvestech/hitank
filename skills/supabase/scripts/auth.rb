# Supabase Management API — Auth via Bearer Token
# Usage: require this file at the beginning of any skill script.
# Provides: supabase_request(method, path, body:) helper
...
unless File.exist?(TOKEN_FILE)
  abort <<~MSG
    Supabase token not found at #{TOKEN_FILE}
    Run: ruby #{File.expand_path('../check_setup.rb', __FILE__)}
  MSG
end

SUPABASE_TOKEN = File.read(TOKEN_FILE).strip
...
def supabase_request(method, path, body: nil)
  ...
  req['Authorization'] = "Bearer #{SUPABASE_TOKEN}"
  if body
