# Save the DigitalOcean API token
...
token_dir  = File.expand_path('~/.config/digitalocean')
...
account = JSON.parse(resp.body).fetch('account', {})
puts "Verified: logged in as #{account['email']}"
